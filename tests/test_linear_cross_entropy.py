...
def test_chunked_linear_cross_entropy(B_L, d, V, chunk_size, reduction, input_dtype):
    ...
    loss_ref = linear_cross_entropy_func_ref(x_ref.float(), weight_ref.float(), None, target, reduction=reduction)
    ...
    assert (weight.grad - weight_ref.grad).abs().max() < 2 * (weight_pt.grad - weight_ref.grad).abs().max() + 1e-4
